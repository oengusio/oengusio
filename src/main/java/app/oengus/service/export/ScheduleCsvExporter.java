...
public class ScheduleCsvExporter implements Exporter {
    ...
    public ScheduleCsvExporter(ScheduleHelper scheduleHelper) {
        this.scheduleHelper = scheduleHelper;
    }

    @Override
	public Writer export(final String marathonId, final String zoneId, final String language) throws IOException {
		final List<ScheduleLineDto> scheduleLineDtos = this.scheduleHelper.getSchedule(marathonId, zoneId).getLines();
		...
		if (!scheduleLineDtos.isEmpty()) {
			try (final CSVPrinter printer = new CSVPrinter(
                ...
                CSVFormat.RFC4180.withHeader(
                    scheduleLineDtos.get(0).getCsvHeaders()
