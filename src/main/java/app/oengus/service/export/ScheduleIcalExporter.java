...
public class ScheduleIcalExporter implements Exporter {
    ...
    public ScheduleIcalExporter(ScheduleHelper scheduleHelper) {
    ...
    public Writer export(final String marathonId, final String zoneId, final String language) throws IOException {
        ...
        final ResourceBundle resourceBundle = ResourceBundle.getBundle("export.Exports", Locale.forLanguageTag(language));
        final Calendar calendar = new Calendar();

        // TODO: get marathon name
        ...
        scheduleDto.getLines().forEach(
            (line) -> calendar.getComponents().add(this.mapLineToEvent(line, timeZone, resourceBundle))
        );

        return new StringWriter().append(calendar.toString());
    }

    private VEvent mapLineToEvent(final ScheduleLineDto scheduleLineDto, final TimeZone tz, final ResourceBundle resourceBundle) {
        final MessageFormat messageFormat = new MessageFormat(resourceBundle.getString("schedule.export.ics.title.pattern"));
        final String[] arguments = {
            scheduleLineDto.getGameName(),
            scheduleLineDto.getCategoryName(),
            scheduleLineDto.getRunners()
                .stream()
                .map(
                    (user) -> user.getUsername(resourceBundle.getLocale().toLanguageTag())
                )
        ...
        final VEvent event = new VEvent(
            new DateTime(Date.from(scheduleLineDto.getTime().toInstant()), tz),
            new DateTime(Date.from(
                scheduleLineDto.getTime().plus(scheduleLineDto.getEstimate()).toInstant()),
                tz
            ),
            title
        );
